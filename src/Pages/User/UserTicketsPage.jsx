...
import { FaDownload, FaTrash, FaExchangeAlt, FaSpinner, FaCalendarAlt, FaMapMarkerAlt, FaTicketAlt } from 'react-icons/fa';
import jsPDF from 'jspdf';
import html2canvas from 'html2canvas';
import { useAuth } from '../../Context/AuthContext';
import { Link } from 'react-router-dom';
import TransferTicketModal from './TransferTicketModal'; // Ensure correct import path

// In JavaScript, we don't use explicit interfaces.
// The structure of the data will be inferred at runtime.

const UserTicketsPage = () => { // Removed React.FC
  const [tickets, setTickets] = useState([]); // Removed type annotation <Ticket[]>
  const { user } = useAuth();
  const [selectedTicketId, setSelectedTicketId] = useState(null); // Removed type annotation <string | null>
  const [showTransferModal, setShowTransferModal] = useState(false); // Removed type annotation <boolean>
  const [loadingTickets, setLoadingTickets] = useState(true); // Removed type annotation <boolean>

  // Function to fetch user's tickets
  const fetchTickets = async () => {
    setLoadingTickets(true);
    try {
      // Assuming this endpoint returns a list of tickets/registrations
      const res = await API.get('/tickets/my-tickets'); // Removed explicit type response
  ...
  const cancelTicket = async (ticketId) => { // Removed type annotation : string
    // IMPORTANT: For better UX, consider replacing window.confirm with a custom modal component
    // that offers more styling control and a more integrated feel.
    if (!window.confirm('Are you sure you want to cancel this ticket? This action cannot be undone and may not be refundable.')) {
      return;
    }

    try {
      // Assuming backend endpoint for cancellation
      const res = await API.delete(`/tickets/cancel/${ticketId}`); // Removed type response
  ...
  const downloadTicket = async (ticket) => { // Removed type annotation : Ticket
  ...
  const formatDateTime = (isoString) => { // Removed type annotation : string
    try {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {tickets.map((ticket) => {
              ...
              return (
                ...
                >
                  {isCancelled && (
                    <span className="absolute top-4 right-4 bg-red-600 text-white text-xs font-bold px-3 py-1 rounded-full uppercase">Cancelled</span>
                  )}
                  {isTransferred && (
                    <span className="absolute top-4 right-4 bg-yellow-600 text-white text-xs font-bold px-3 py-1 rounded-full uppercase">Transferred</span>
                  )}

                  <h3 className="text-2xl font-bold text-gray-900 mb-2">{ticket.event.title}</h3>
                  <p className="text-gray-700 text-base mb-1">
                    <span className="font-semibold">{ticket.ticket.type} Ticket</span>
                  ...
                  <p className="text-gray-800 text-lg font-bold mb-3">
                    Price: ₹{ticket.totalAmount ? ticket.totalAmount.toFixed(2) : '0.00'}
                  </p>
